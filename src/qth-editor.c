...
/**
 * Symbolic references used when calling select_location in order to determine
 * which mode the selection should run in (location or wx station).
 */
enum {
 ...
 * will run in edit mode, i.e. preload the widgets with values from qth,
 * otherwise it will create empty widgets.
 *
 * To see the widget in action click on the "+" button in either the
 * "New Module" or "Configure Module" dialog.
 *
 * @bug Edit mode doesn't really works, that is, it does, but probably leaks
 *      memory because I can't free existing data in qth_t
 */
...
static GtkWidget *create_editor_widgets(qth_t * qth)
{
    ...
    locbut = gpredict_hstock_button(GTK_STOCK_INDEX, _("Select"),
                                    _("Select a predefined location from "
                                      "a list."));
    ...
    /* connect lat/lon spinners and combos to callback remember signal id
     * so that we can block signals while doing automatic cross-updates */
    ...
    gtk_widget_set_tooltip_text(alt,
                                _("Select the altitude of the ground station "
                                  "in meters or feet depending on your settings"));
    ...
    wxbut = gpredict_hstock_button(GTK_STOCK_INDEX, _("Select"),
                                   _("Select a predefined weather station "
                                     "from a list."));
    ...
    gtk_widget_set_tooltip_text(type,
                                _("A location can be static, i.e. it does not "
    ...
    gtk_widget_set_tooltip_text(port,
                                _("Set the port for GPSD to use. "
                                  "Default for gpsd is 2947."));
...
/**
 * Apply changes.
 * @param qth Pointer to the QTH handle.
